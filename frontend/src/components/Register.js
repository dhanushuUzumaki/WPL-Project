...
function Register(props) {
  ...
  const submitHandler = (e) => {
    ...
    if (!password.match(pattern) || name.length === 0 || email.length === 0) {
    ...
    <form onSubmit={submitHandler} >
      <ul className="form-container">
        ...
        <li>
          ...
          <input type="password" id="password" name="password" onChange={(e) => setPassword(e.target.value)} required="true">
          </input>
        </li>
        <li>
          <label htmlFor="rePassword">Re-Enter Password</label>
          <input type="password" id="rePassword" name="rePassword" onChange={(e) => setRePassword(e.target.value)} required="true">
          </input>
        </li>
        <li>
